...
must_haves:
  truths:
    - "setMarkedText updates CompositionState preedit"
    - "insertText commits text and clears composition"
    - "unmarkText cancels composition without commit"
    - "hasMarkedText returns true during composition"
    - "markedRange returns preedit byte range"
  artifacts:
...
<objective>
Implement NSTextInputClient core methods in overlay to bridge IME events to V CompositionState.

Purpose: Enable Japanese/Chinese IME to communicate preedit/commit/cancel to VGlyph.
Output: Working setMarkedText/insertText/unmarkText forwarding to V callbacks.
</objective>
...
`v -check-syntax c_bindings.v` passes (no V changes yet, just verify no breakage)
  ...
  <done>
setMarkedText forwards to callback, markedRange/selectedRange/hasMarkedText return correct state
  </done>
</task>
...
grep finds `unmarkText` clearing _markedRange
  ...
  <done>
insertText commits and clears, unmarkText cancels, V bindings expose callback registration
  </done>
</task>
...
<success_criteria>
- setMarkedText extracts text and calls on_marked_text callback
- insertText extracts text, calls on_insert_text, calls unmarkText
- unmarkText resets _markedRange, calls on_unmark_text
- hasMarkedText returns true during composition
