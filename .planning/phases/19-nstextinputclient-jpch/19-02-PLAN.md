...
must_haves:
  truths:
    ...
    - "Japanese clause segmentation uses thick/thin underlines"
    - "Chinese pinyin shows preedit with candidate selection"
  ...
  artifacts:
    ...
    - path: "ime_overlay_darwin.h"
      provides: "Bounds callback declaration"
      contains: "on_get_bounds"
    - path: "c_bindings.v"
      provides: "V bounds callback registration"
      contains: "IMEOverlayBoundsCallback"
  key_links:
    - from: "ime_overlay_darwin.m"
      to: "V CompositionState.get_composition_bounds"
      via: "bounds callback"
...
Purpose: Enable IME candidate window to appear near cursor; parse clause info for underline styles.
Output: Working firstRectForCharacterRange with proper screen transforms, clause parsing.
</objective>
...
2. Add callback type: `type IMEOverlayBoundsCallback = fn (user_data voidptr, x &f32, y &f32, w &f32, h &f32) bool`
...
`v -check-syntax c_bindings.v` passes
  ...
  <done>
Clause attributes parsed from NSAttributedString, reported via callbacks to V
  </done>
</task>
...
<success_criteria>
- firstRectForCharacterRange returns screen coordinates from V bounds callback
...
- Thick underline = selected clause style
- Out-of-bounds ranges return NSZeroRect (no crash)
