...
**Clarification: cancel vs commit**
- **Cancel** (composition.cancel()): Discards preedit text entirely. The composing syllable
  (e.g., 간) disappears without being inserted. Used for: Escape key, Option+Backspace.
- **Commit** (composition.commit() or unmarkText): Inserts preedit text permanently. The
  composing syllable becomes real text. Used for: focus loss, space key, Cmd+A.
